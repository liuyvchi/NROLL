...
def extract_features(pretrained_model_path, test_data_loader, label2files, output_dir, p=1, t=2, r=1, net_mode='irse'):
    ...
    # output_path = os.path.join(output_dir, 'deepdlint.pkl')
    # output = open(output_path, 'wb')
    # pickle.dump(label2files, output)
    # output.close()
    # logger.info('Dump finished!')

    with open ('deepdlint.pkl', 'rb') as f:
        label2files= pickle.load(f)
    ...
    for label in label2files:
        ...
        score = np.triu(sim, 1) 
        score = score[score != 0]
        score = score[(-1<score)*(score<0.2)]
        total_noise.extend(score.tolist())
    print(len(total_noise))
    logger.info('There are %d noise scores.' % len(total_noise))
    plt.hist(total_noise, bins=10000)
    plt.xlabel('cos')
    plt.ylabel('#of examples')
    plt.title('noise score distribution')
    plt.savefig('noise_deepglint.jpg')
