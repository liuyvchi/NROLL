...
def train_one_split(conf, data_loader, org_t, loss_fn, models, fc_ms, optimizers, saved_dir, print_freq=200):
    # switch to train mode
    device = torch.device('cuda:0')
    M = conf.M
    alpha = conf.alpha
    
    check_point_size = (conf.iter_per_split // 3, conf.iter_per_split // 2, conf.iter_per_split // 3 * 2)

    data_iter = iter(data_loader)

    for batch_idx in range(conf.iter_per_split): #conf.full_loader_size
        try:
        ...
        cur_iteration = conf.cur_iteration + batch_idx

        ## warm up r0##
        r_e = min(((cur_iteration+1)/(3*conf.full_loader_size))*conf.noise_rate, conf.noise_rate)
        conf.r0 = (1-r_e)
        ##
        ## warm up on t##
        t = min(((cur_iteration+1)/(conf.epochs*conf.full_loader_size))*org_t, org_t)
        ##
        ...
        for i in range(M):
            ...
            if batch_idx % print_freq == 0:
                ...
                print('epoch %d, iter %d, lr %f, model_m%f, loss_clean %f' % (conf.cur_split, batch_idx, lr, i, loss_clean)) 
                print('epoch %d, iter %d, lr %f, model_m%f, loss_hc %f' % (conf.cur_split, batch_idx, lr, i, loss_hc)) 
                print('epoch %d, iter %d, lr %f, model_m%f, loss_m %f' % (conf.cur_split, batch_idx, lr, i, loss_m_val))   
            ...
            if batch_idx != 0 and batch_idx in check_point_size:
                if conf.cur_split > conf.cur_split - 5:
                    saved_name = 'model_%d_epoch_%d_batch_%d.pt' % (i, conf.cur_split, batch_idx)
                ...
                if i == 0:
                    ...
                    lfw = res[0]
                    print("lfw:", lfw, "cur_split_batch:", conf.cur_split, batch_idx)
                    conf.writer.add_scalar('lfw_m:', lfw, cur_iteration)
        
            conf.lr_schedules[i].step()

    for i in range(M):
        saved_name = 'model_%d_split_%d.pt' % (i, conf.cur_split)
        torch.save({'state_dict': models[i].module.state_dict()}, os.path.join(saved_dir, saved_name))
        print('save checkpoint %s to disk...' % (saved_name))


def first_annotation(conf, labeld_ind, full_db):
    with open (os.path.join(conf.saved_dir, "annotation.txt"), 'w') as f:
        for i in labeld_ind:
            k, v = full_db.train_list[i][0], full_db.train_list[i][1]    
            f.write('{} {}\n'.format(k, v))

def annotation(conf, loader_unlabeled, models, fc_ms):
    M = conf.M
    loader_size = len(loader_unlabeled)
    check_point_size = (loader_size // 3, loader_size // 2, loader_size // 3 * 2)
    writer = {}
        
    with open (os.path.join(conf.saved_dir, "annotation.txt"), 'r') as f:
        lines = f.readlines()
        for l in lines:
            k, v = l.split(' ')
            writer[k] = int(v)

    for batch_idx, (image, label, lmdb_keys) in enumerate(loader_unlabeled):
        image = image.cuda()
        max_cos = torch.zeros(len(lmdb_keys)).cuda()
        pred_label = torch.zeros(len(lmdb_keys)).cuda()
        for i in range(M):
            models[i].eval()
            feat_norm = models[i](image)
            pred = cos_value(feat_norm, fc_ms[i])
            pred_max_m, label_max_m = torch.max(pred, 1)

            for j in range(len(label)):
                if pred_max_m[j] > max_cos[j]:
                    max_cos[j] = pred_max_m[j].item()
                    pred_label[j] = label_max_m[j].item()

        for j in range(len(label)):   
            if max_cos[j] > 0.65:              
                writer[lmdb_keys[j]] = int(pred_label[j].item())
    
    with open (os.path.join(conf.saved_dir, str(conf.cur_split)+"annotation.txt"), 'w') as f:        
...
#     db = lmdb_utils.MultiLMDBDataset_noise(conf.source_lmdb_list, conf.source_file_list, conf.num_class, conf.noise_rate, conf.key) 
    ...
    full_db = lmdb_utils.MultiLMDBDataset(conf.source_lmdb_list, conf.source_file_list, conf.key)
    loader_full = DataLoader(full_db, conf.batch_size, True, num_workers=4)
    conf.full_loader_size = len(loader_full)
    #split_results = lmdb_utils.semi_split_n(full_db.labels, conf.num_class, conf.split_num)
    #with open('msc-R_split3.pickle', 'wb') as f:
    #    pickle.dump(split_results, f)
    #assert(0)
    with open('msc-R_split3.pickle', 'rb') as f:
        split_results = pickle.load(f)

    indexs_labeled = split_results[0]
    indexs_unlabeled = split_results[1:]
    ...
    conf.lr_schedules = []
    conf.milestones = [p*conf.full_loader_size for p in conf.milestones]
    for i in range(conf.M):
        lr_schedule = optim.lr_scheduler.MultiStepLR(optimizers[i], milestones=conf.milestones, gamma=0.1)
        conf.lr_schedules.append(lr_schedule)
        
    conf.real_sample = 0
    
    conf.iter_per_split = int(conf.epochs*conf.full_loader_size/conf.split_num) + 1
    print("iter_per_split:", conf.iter_per_split)
    conf.cur_split = 0
    conf.cur_iteration = 0 
    noise_sample = len(indexs_labeled)*conf.noise_rate
    for split_id in range(conf.split_num):
        
        if split_id == 0:  
            first_annotation(conf, indexs_labeled, full_db)

        new_file_list = [os.path.join(conf.saved_dir, "annotation.txt")]
        db_labeled = lmdb_utils.MultiLMDBDataset(conf.source_lmdb_list, new_file_list, conf.key)
        ...
        # conf.r0 = (1-r_e)
        print("r0:", conf.r0)
        train_one_split(conf, loader_labeled, conf.t[conf.cur_split], loss_fn, models, fc_ms, optimizers, conf.saved_dir, conf.print_freq)
        conf.cur_split +=1 
        conf.cur_iteration += conf.iter_per_split
        print("conf.cur_split, conf.cur_iteration:", conf.cur_split, conf.cur_iteration)
        
            

        if split_id < conf.split_num-1:
            db_unlabeled = lmdb_utils.MultiLMDBData_semi(conf.source_lmdb_list, conf.source_file_list, indexs_unlabeled[split_id], conf.key)
            loader_unlabeled = DataLoader(db_unlabeled, conf.batch_size, True, num_workers=4)
            annotation_len = annotation(conf, loader_unlabeled, models, fc_ms)
            db_unlabeled.close()
        
        print("label samples in last stage:", annotation_len)
        conf.r0 = 1-noise_sample/annotation_len
        db_labeled.close()
