...
def update_model_k(model_q, model_k, m):
    for q_param, k_param in zip(model_q.parameters(), model_k.parameters()):
        k_param.data.mul_(m).add_(1 - m, q_param.data)
...
def clustering(conf, labeled_loader, unlabel_loader, loss_fn, model_q, model_k, pre_centers, optimizer, saved_dir, print_freq=200, introspection_freq=1000):
    ...
    for batch_idx in range(conf.total_iter): #conf.full_loader_size
        try:
            img_lq, img_lk, label, lmdbkeys_1, lmdbkeys_2 = labeled_iter.next()
        except StopIteration:
            labeled_iter = iter(labeled_loader)
            img_lq, img_lk, label, lmdbkeys_1, lmdbkeys_2 = labeled_iter.next()

        try:
            img_uq, img_uk, label_u, lmdbkeys_u = unlabeled_iter.next()
        except StopIteration:
            unlabeled_iter = iter(unlabel_loader)
            ...
            unlabel_epoch +=1
            if unlabel_epoch%2==0 and unlabel_epoch>0:
                cluster_dic = {}
        ...
        img_q = torch.cat((img_lq, img_uq), dim=0).cuda()
        feat_q = model_q(img_q) 
        feat_l_q = feat_q[:len(img_lq)]
        feat_u_q = feat_q[len(img_lq):]
        
        model_k.eval()
        img_k = torch.cat((img_lk, img_uk), dim=0).cuda()
        model_k.eval()
        def set_bn_train(m):
        ...
        with torch.no_grad():            
            feat_k = model_k(img_k) 
        feat_l_k = feat_k[:len(img_lk)]
        feat_u_k = feat_k[len(img_lk):]

        # compute loss for labeled data
        logits_l = torch.mm(feat_l_q, prototypes.transpose(0,1).detach()) 
        logits_l = logits_l.clamp(-1, 1)
        logits_l = torch.div(logits_l, 0.033) 
        loss_l = loss_fn(logits_l, label)
        
        # compute loss for unabeled data
        logits_u = torch.mm(feat_u_q, prototypes.transpose(0,1).detach())
        logits_max, pred_u = torch.max(logits_u, dim=1)
        idx_old, idx_new, T_up, T_down = threshould.get_T_v2(feat_l_q.detach(), feat_u_q.detach(), label.detach(), prototypes.detach(), T_up, T_down, m_t)
        conf.writer.add_scalar('T_up', T_up, batch_idx)
        conf.writer.add_scalar('T_down', T_down, batch_idx)
        
        # compute loss for unlabeled old classes data 
        if len(idx_old) > 0:
            logits_u_old = logits_u[idx_old]
            logits_u_old = logits_u_old.clamp(-1, 1)
            logits_u_old = torch.div(logits_u_old, 0.033) 
            loss_u_old = loss_fn(logits_u_old, pred_u[idx_old])
        else:
            loss_u_old = torch.tensor(0).cuda()

        # compute loss for unlabeled new classes data 
        if len(idx_new) > 0:
            N = len(idx_new)
            C = conf.feat_dim
            K = len(prototypes)
            logits_p = torch.bmm(feat_u_q[idx_new].view(N,1,C), feat_u_k[idx_new].view(N, C, 1)).view(N, 1)   # positive logits
            logits_n = torch.mm(feat_u_q[idx_new].view(N,C), prototypes.transpose(0,1).detach().view(C, K)) # negative logits
            logits_u_new = torch.cat((logits_p, logits_n), dim =1) 
            logits_u_new = logits_u_new.clamp(-1, 1)
            logits_u_new = torch.div(logits_u_new, 0.033) 
            label_u_new = torch.zeros(N).long().cuda() # label for new classes are all 0-th
            loss_u_new = loss_fn(logits_u_new, label_u_new)
        else:
            loss_u_new = torch.tensor(0).cuda()

        # total loss
        loss = loss_l + 0.5*loss_u_old + 0.5*loss_u_new
        
        conf.writer.add_scalar('loss', loss, batch_idx)
        conf.writer.add_scalar('loss_l', loss_l, batch_idx)
        conf.writer.add_scalar('loss_u_old', loss_u_old, batch_idx)
        conf.writer.add_scalar('loss_u_new', loss_u_new, batch_idx)
        if batch_idx % print_freq == 0:
            lr = get_lr(optimizer)
            print('introspection %d, iter %d, lr %f, loss %f， loss_l %f, loss_u_old %f， loss_u_new %f' % (introspection_times, batch_idx, lr, loss, loss_l, loss_u_old, loss_u_new))
        ...
        # update cluster_dic
        for i in idx_old.tolist():
            key = lmdbkeys_u[i]
            i_pred = pred_u[i].item()     
            cluster_dic[key]= i_pred 
        for i in idx_new.tolist():
            key = lmdbkeys_u[i]
            i_label = len(prototypes)+i 
            cluster_dic[key] = i_label
        conf.writer.add_scalar('clustering_number', len(cluster_dic), batch_idx)
        
        # update prototypes
        prototypes[label] = m_p*prototypes[label] + (1-m_p)*feat_l_k.detach() # update old prototypes in the momentum fashion by using labeled data
        prototypes[label] = F.normalize(prototypes[label], dim=1)
        del feat_l_k
        prototypes[pred_u[idx_old]] = m_p*prototypes[pred_u[idx_old]] + (1-m_p)*feat_u_k[idx_old].detach() # update old prototypes in the momentum fashion by using unlabeled data
        prototypes[pred_u[idx_old]] = F.normalize(prototypes[pred_u[idx_old]], dim=1)
        if len(idx_new) > 0 :
            prototypes = torch.cat((prototypes, feat_u_k[idx_new].detach()), 0)   # add new class prototypes:
        del feat_u_k 
        conf.writer.add_scalar('prototypes_length', len(prototypes), batch_idx)

        if batch_idx % conf.test_freq == 0:
            print("test model on LFW")
            model_q.eval()
            res = test(model_q, lfw_test_pairs, test_data_loader, blufr=True)
            lfw = res[4]
            print("lfw:", lfw, "cur_introspection_batch:", introspection_times, batch_idx)
            conf.writer.add_scalar('lfw1e-5', lfw, batch_idx)
...
def train(conf):
    ...
    parameter = [p for p in model_q.parameters() if p.requires_grad]
    ...
    db_unlabeled = lmdb_utils.SingleLMDBDataset_2aug(conf.source_lmdb_list[0], unlabeled_file_list[0], conf.key)
...
#     conf.add_argument("--train_db_name", type=str, default='vggface2_part34_lmdb', help="comma separated list of training database.")
    ...
    conf.add_argument('--lr', type=float, default=0.001, help='initial learning rate.')
